...
### 2026-06-12 (session 40, training voice + daily thoughts + Best Moment + kids polish; beta-guide gap flagged)
- **Training rewrite (app)**: Range, Short Game, Strategy now carry a metaphor/visualization layer across all levels (fuller for beginners), sourced from `pga-coaching-reference-v1.md` (Teaching Cues by Audience + Content Style Guidelines). Includes Jeremy's reframe: "your next shot is the only one that matters, take a breath and move on." Voice approved via samples before applying.
- **Daily thoughts feed (app)**: new "Today" section on Home rotates 2 date-seeded cards from an 18-tip universal pool (`DAILY_THOUGHTS`, drills/strategy/mental/gear) + a lefty bonus tip. Replaces the single static "Today's Focus" line. Verified live.
- **Best Moment share template (app)**: `buildBestMomentCard()` auto-picks the round's standout hole (best vs par, photo breaks ties) → hero photo + big score + achievement label + logo/frame. App sharing now at full PWA parity (3 templates). Verified live (picked the birdie-with-photo).
- **App layout**: reviewed screen-by-screen at mobile width; clean/consistent, no blind changes (awaiting Jeremy's specific targets).
- **PWA kids' training polish**: kids' content was already strongly in the game/metaphor voice (son range "Make It Fly!/Rocket Launch", girl "Magic Swings/SWOOOOOP"). Targeted only: son short-game chip → "putt it with a 9-iron… wrists like little statues"; girl "shaft lean" jargon → "hands leading the club"; **fixed stale Daughter "Age 5" → "Age 6"** (range tag + caddie context). Verified live.
- SW bumps: app v4→v5; PWA cache v152→v153 (sw v106→v107).
- **⚠ GAP FOUND → DECISION MADE (next sprint):** `beta-user-guide.md` Step 2 promises testers family rounds, "up to 5 profiles," kid leveling, and game modes "in app.spinvibes.com", but the **public app has none of these today** (solo-only; 0 family/kid code). The kid system + metaphor content exist **only in the PWA, hardcoded to Jeremy's Son/Daughter** (`FAM_PLAYERS` fixed keys, 32 `son-sect`/`girl-sect` blocks). **Jeremy's call (2026-06-12):** build a family/kid system into the app where **each added member gets a CUSTOM plan exactly like the main user**: dynamic and age/hand/skill-keyed (not hardcoded), with the kid metaphor/game voice, just like the PWA. Next sprint: load wizard family + per-member custom plan + profile switcher + **in-app add/edit family (post-wizard, Settings → Family)** + age-keyed training/daily-thoughts. **Plus an honesty mechanism:** an in-app **"Coming at launch" list** that openly shows features promised but not yet in beta (family rounds, game modes, kid leveling, etc.), framed as available at launch, shrink it daily as features ship, never over-state. Full scope + traced data shapes + the kickoff prompt: `CURRENT-STATUS.md` → NEXT SPRINT, and `SPRINT-PROMPT-family-app.md`.

### 2026-06-12 (session 39, app↔PWA parity + nav/overlap fixes + beta docs email)
- **Coach's Plan markdown (spinvibes-app):** `coaching_plan` rendered raw `#`/`**`. Added a lightweight `mdToHtml()` (headings, bold, italics, bullet + numbered lists, paragraphs) + `.md` styles; Home now renders it formatted. Unit-tested in Node.
- **Handicap index tracker ported to app (My Game):** `_scoreDiff` / `_whsBestCount` / `calcHandicapIndex` / `handicapCardHtml`, WHS estimate (best-N differentials × 0.96), 8-bar sparkline, "rounds used of eligible." Rounds now **capture course rating/slope** from the selected tee at start (`_round.rating/slope`) and persist them on the saved round; only `regulation_18` rounds with rating+slope are eligible (par-3/9-hole excluded). Card hidden until there's ≥1 eligible round.
- **Scorecard OCR ported to app (round setup):** "OR SCAN A SCORECARD", tee-row pills + take/upload photo → resize to 1400px → `/parse-scorecard` worker → pre-fills course name, holePars, yardages, par, rating/slope and shows it as the selected course so you can Start Round. Worker already allows `app.spinvibes.com` origin (no worker redeploy). Verified live.
- **Editable OCR review (follow-up, same session):** after parse, `renderScanReview()` shows an editable table (course name + per-hole par/yards, live "Total par") so misreads are corrected before Start Round, full PWA parity. `scanEditHole()` writes edits into `_setupCourse`; `clearScanReview()` (✕) resets. Verified live (incl. live total recompute).
- **Handicap backfill (follow-up, same session):** regulation-18 rounds missing rating/slope show a "+ Handicap data" chip in My Game → bottom sheet (`openHandicapFix`/`saveHandicapFix`) to enter rating (50–90) + slope (55–155). On save, round becomes eligible and the index recomputes. `_hcEligible(r)` gates the chip (18-hole, no rating/slope). Verified live, eligible count went 3→4. (Older pre-session rounds need this to count; new rated rounds capture it automatically.)
- **Caddie bottom-nav overlap fixed (spinvibes-app):** `.caddie-outer` used a fixed `height: calc(100dvh - 110px …)` inside a `.page` that also had the title + bottom padding, pushing the input row **under the nav** (invisible). Rebuilt as flex: `#pg-caddie.on{display:flex;flex-direction:column}`, `.caddie-outer{flex:1;min-height:0}`, msgs scroll, input pinned above nav. Verified live.
- **Short Game back button** added to both apps (was a dead-end, no way back to Range): app gets a `.back-chip` "‹ Range" (+ Strategy → My game); PWA `page-shortgame` header gets a "‹ Range" button matching the caddie page pattern. Verified live on both.
- **PWA mobile nav overlap fixed:** with global `box-sizing:border-box`, `#nav{height:54px;padding-bottom:env(safe-area-inset-bottom)}` let the safe-area inset eat into the 54px, so the 54px `.nav-btn`s overflowed **upward** over page content on notched phones. Changed to `height: calc(var(--nav-h) + env(safe-area-inset-bottom))` (matches app's nav). Desktop ≥960px sidebar override unaffected.
- **Overlay/close audit:** app share sheets got an ✕ (already closed on tap-out); confirmed settings/round/family/add-course/image-viewer/pin/round-edit modals all already have close or back affordances on both apps. Only Short Game lacked one.
- **Beta docs email:** `BETAUSERDOCS` contact changed `jeremy.gelbaum@gmail.com` → `spinvibespb@gmail.com` in `beta-user-guide.md` and in `SpinVibes-Beta-Invite.pdf` + `.png` (PDF text redacted/reinserted via PyMuPDF, PNG re-rendered from the corrected PDF at original 2160×4180, design preserved).
- **SW bumps:** spinvibes-app v2→v3; PWA cache v151→v152 (sw v105→v106).
- **Meta backup push:** was failing on GitHub secret-scanning, `spinvibes-meta/SESSION-START.md` had the live GitHub PAT in plaintext (lines 110, 129). Redacted to match the sanitized root copy; backup now pushes. **TODO (security, deferred):** rotate the PAT and purge it from local remote URLs / `push-*.command` / meta git history.
